...
struct CRTTriangle {
	...
	static long long intersectRayWithTraingles(
		const CRTRay& ray,
		...
		CRTVector bestP{ std::numeric_limits<float>::max(),
				...
				std::numeric_limits<float>::max() };
		long long closestTriangleIdx = -1;
		...
		for (size_t i = 0; i < len; i++) {
			...
			if (bestP.length() > p.length()) {
				...
				partialOverlap = true;
				returnColor.r = (returnColor.r + colors[i].r) / 2;
				returnColor.g = (returnColor.g + colors[i].g) / 2;
				returnColor.b = (returnColor.b + colors[i].b) / 2;
			}
		}
		return partialOverlap ? -2 : closestTriangleIdx;
	}
};

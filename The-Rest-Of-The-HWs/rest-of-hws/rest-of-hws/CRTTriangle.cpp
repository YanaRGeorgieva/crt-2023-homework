...
CRTVector CRTTriangle::intersect(const CRTRay& ray) const {
	// If R is not parallel to the triangle’s plane : dot(N, R) != 0
	const float rProj = triNormalVector.dot(ray.direction);
	if (equals(rProj, 0.0f)) {
		return worstP;
	}

	// Calculate distance to triangle's plane
	const float rpDist = (v0 - ray.origin).dot(triNormalVector);
	...
	if (dotTriNormalWithe0xv0p < 0.0f) {
		return worstP;
	}
	...
	if (dotTriNormalWithe1xv1p < 0.0f) {
		return worstP;
	}
	...
	if (dotTriNormalWithe2xv2p < 0.0f) {
		return worstP;
	}
